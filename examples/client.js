...
{
  captchaSet = true;
  captchaArg = args[3].toString();
}

console.log(`client.js - captchaArg: ${captchaArg}`);
...
switch (args[0]) {
  ...
  case 'requestSMS':
    ...
    accountManager
      .requestSMSVerification(captchaArg)
      .then(result => {
  ...
  case 'requestVoice':
    username = args[1];
    password = args[2];
    console.log(`client.js - requestVoice - captchaArg: ${captchaArg}`);
    accountManager = new Signal.AccountManager(
      username,
      password,
      protocolStore      
    ...
    accountManager
      .requestVoiceVerification(captchaArg)
      .then(result => {
